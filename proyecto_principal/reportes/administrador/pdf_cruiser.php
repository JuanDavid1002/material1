...
$pdf->Cell(28,10,'Precio',1,1,'C');
 ...
 while ($row = $result->fetch_assoc()) {
    ...
    $precio=$row["precio"];
    $simbolo='$';
    $pdf->Cell(28,10, $row["referencia_cruiser"],1,0,'C',0);
    $pdf->Cell(28,10, $row["color_cruiser"],1,0,'C',0);
    $pdf->Cell(28,10, $row["modelo_cruiser"],1,0,'C',0);
    $pdf->Cell(28,10, $row["marca_cruiser"],1,0,'C',0);
...
$consulta1 = "SELECT SUM(totalmotos), FORMAT(SUM(totalmotos*precio),0) FROM tab_motos_cruiser";
...
$pdf->Cell(40,10,'total Dinero',1,1,'C');
$consulta1 = "SELECT SUM(salidamotos),  FORMAT(SUM(salidamotos*precio),0) FROM tab_motos_cruiser";
if ($resultado = $conn->query($consulta1)) {
while ($fila = $resultado->fetch_row()) {
    $fila[0]; 
    $fila[1]; 
