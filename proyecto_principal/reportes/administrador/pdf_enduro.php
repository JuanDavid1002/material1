...
function Header()
{
    ...
    $this->Cell(40);
    // Título
    $this->Cell(120,10,'Reporte de Motos de Tipo Enduro',1,0,'C');
    // Salto de línea
    $this->Ln(30);
}
...
$consulta="SELECT * FROM tab_motos_enduro";
...
$pdf->Cell(28,10,'Precio',1,1,'C');
 ...
 while ($row = $result->fetch_assoc()) {
    ...
    $precio=$row["precio"];
    $simbolo='$';
    $pdf->Cell(28,10, $row["referencia_enduro"],1,0,'C',0);
    $pdf->Cell(28,10, $row["color_enduro"],1,0,'C',0);
    $pdf->Cell(28,10, $row["modelo_enduro"],1,0,'C',0);
    $pdf->Cell(28,10, $row["marca_enduro"],1,0,'C',0);
...
$consulta1 = "SELECT SUM(totalmotos), FORMAT(SUM(totalmotos*precio),0) FROM tab_motos_enduro";
...
$pdf->Cell(40,10,'total Dinero',1,1,'C');
$consulta1 = "SELECT SUM(salidamotos),  FORMAT(SUM(salidamotos*precio),0) FROM tab_motos_enduro";
if ($resultado = $conn->query($consulta1)) {
while ($fila = $resultado->fetch_row()) {
    $fila[0]; 
    $fila[1]; 
