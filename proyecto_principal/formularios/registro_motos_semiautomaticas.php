...
$stmt = $conn->prepare("CALL ingresar_motos_semiautomaticas (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$stmt->bind_param("ssssiisisssssssssssssssssssssssssssss", $referencia,$color,$modelo,$marca,$cantidad,$precio,$cilindraje,
$clasificacionmotos,$transmision,$motor,$suspensiondelantera,$suspensiontrasera,$diametrocarrera,
$relacioncomprension,$sistemavalvulas,$potenciamaxima,$torque,$alimentacion,$aceitetransmision,
$sistemalubricacion,$ignicion,$sistemaarranque,$sistemaencendido,$distanciaejes,$largo,$llantadelantera,
$llantatrasera,$ancho,$altura,$frenodelantero,$frenotrasero,$alturaasiento,$distanciapiso,$pesoseco,$longitudtotal,
$alturatotal,$capacidadtanque);
...
?>
 ...
 <input type="submit" name="select" class="btn btn-success btn-lg" value="Nuevo Registro" onclick="funcion(), location.href='formulario_motos_semiautomaticas.html'"/>
 <input type="submit" name="select" class="btn btn-success btn-lg" value="Regresar" onclick="funcion(), location.href='/material1/template.php'"/>

<script>
  function funcion(){
    document.write('<?php echo select1(); ?>');
  }
</script>     
